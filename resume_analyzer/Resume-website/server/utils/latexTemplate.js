// LaTeX resume template utilities
...
const generateHeader = (personalInfo) => {
  const { name, email, phone, linkedin, github, portfolio, leetcode } =
    personalInfo;
  ...
  if (email) {
    contactLinks.push(
      `\\faIcon{envelope}\n    \\href{mailto:${email}}{\\color{black}${escapeLatex(
        email
      )}}`
    );
  }
  if (phone) {
    contactLinks.push(
      `\\faIcon{phone}\n    \\href{tel:${phone}}{\\color{black}${escapeLatex(
        phone
      )}}`
    );
  }
...
const generateEducationSection = (education) => {
  if (
    ...
    !education.some((e) => e && e.institution)
  )
    return "";
    ...
    .map((edu) => {
      const coursework =
        edu.coursework && edu.coursework.trim() !== ""
          ? `\n    \\resumeItemListStart\n      \\item {\\textit{Relevant Coursework:} ${escapeLatex(
              edu.coursework
            )}}\n    \\resumeItemListEnd\n    \\vspace{-7pt}`
          : "";
      ...
      const cgpaText =
        edu.cgpa && edu.cgpa.trim() !== ""
          ? `\\textbf{CGPA: ${escapeLatex(edu.cgpa)}}`
          : "";
  ...
  const experienceEntries = experience
    ...
    .map((exp) => {
      const achievements = (exp.achievements || []).filter(
        (ach) => ach && typeof ach === "string" && ach.trim() !== ""
      );
    ...
    {${escapeLatex(exp.position || "")}}{${escapeLatex(
        exp.location || ""
      )}}${achievementsList}`;
    })
    .join("\n\n");
...
const generateProjectsSection = (projects) => {
  if (!projects || !Array.isArray(projects) || !projects.some((p) => p && p.name))
    return "";
...
\\vspace{-8pt}`;
  ...
  const projectEntries = projects
    ...
    .map((proj) => {
      const descriptions = (proj.description || []).filter(
